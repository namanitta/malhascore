            ...
            for key in keys:
                ...
                if isinstance(val, int) or isinstance(val, long):
                    ...
                    if key == 'TimeDateStamp' or key == 'dwTimeStamp':
                        try:
							val_str = ""
							val_str += '%s' % time.asctime(time.gmtime(val))
                ...
                if isinstance(val, int) or isinstance(val, long):
                    if key == 'TimeDateStamp' or key == 'dwTimeStamp':
                        try:
                            ...
                            val = '%s ' % (time.asctime(time.gmtime(val)))
                        except exceptions.ValueError, e:
...
class SectionStructure(Structure):
    ...
    def get_hash_ssdeep(self):
        """Get the ssdeep of the section's data."""
        return ssdeep.hash( self.get_data() )
...
class PE:
    ...
    def dump_info(self, dump=None):
        ...
        for section in self.sections:
            ...
            dump.add_line('Ssdeep Hash: %s' % section.get_hash_ssdeep() )
            dump.add_newline()
                ...
                for symbol in module.imports:
					...
					if symbol.import_by_ordinal is True:
						if symbol.name is not None:
							dump.add('%s.%s'% (module.dll, symbol.name))
							#Ordinal[%s] (Imported by Ordinal)'  str(symbol.ordinal)))
						else:
							dump.add('%s' % (module.dll))#Ordinal[%s] (Imported by Ordinal)' % ( module.dll, str(symbol.ordinal)))
					else:
						dump.add('%s.%s' % (module.dll, symbol.name))#, str(symbol.hint)))
        ...
        if hasattr(self, 'DIRECTORY_ENTRY_DELAY_IMPORT'):
            ...
            for module in self.DIRECTORY_ENTRY_DELAY_IMPORT:
                ...
                for symbol in module.imports:
                    if symbol.import_by_ordinal is True:
						dump.add_newline()
						dump.add('%s Ordinal' % (module.dll))#[%s] (Imported by Ordinal)' % (#module.dll, str(symbol.ordinal)))
                    else:
                        dump.add('%s.%s' % (
